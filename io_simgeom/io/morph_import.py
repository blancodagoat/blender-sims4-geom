...
class SIMGEOM_OT_import_morph(Operator, ImportHelper):
    ...
    def execute(self, context):
        ...
        if geom_obj == None:
            ...
            return {'CANCELLED'}
        if not geom_obj.get('__S4_GEOM__'):
        ...
        for geom_file in self.files:
            ...
            try:
                ...
                self.report({'ERROR'}, f"Failed to load morph file {geom_file.name}: {str(e)}")
                continue

            if len(morph_geomdata.element_data) != len(geom_mesh.vertices):
                self.report({'ERROR'}, f"Vertex count mismatch for {geom_file.name}, can't import morph")
                continue
