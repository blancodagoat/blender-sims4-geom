...
class SIMGEOM_PT_utility_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if Globals.OUTDATED == 1:
            ...
            col.operator("wm.url_open", text="NEW VERSION AVAILABLE").url = "https://modthesims.info/"
        elif Globals.OUTDATED == -1:
            col = layout.column(align=True)
            col.label(text="Failed to check for updates.")
        ...
        if not obj is None and obj.type == 'ARMATURE' and obj.get('__S4_RIG__', 0):
            col.operator("simgeom.rebuild_bone_database")

        if obj is not None and obj.get('__S4_GEOM__', 0):
            col.operator("simgeom.import_morph", text="Import Morphs", icon='IMPORT')
            self.draw_geom(context)
        
        if obj is not None and obj.get('__S4_GEOM_MORPH__', 0):
            self.draw_morph(context)
        
        if obj is not None and obj.get('__S4_GEOM_MORPH__') == None:
